...
from spice import SpiceEstimator, Agent, csv_to_dataset, SpiceConfig
...
# get coefficients storage; 69 candidate terms in workingmemory_reward_binary
true_coefs = np.zeros((len(participants), participants[-1]*iterations, 69))
fitted_coefs = np.zeros((len(participants), participants[-1]*iterations, 69))
...
def count_active_params(dict_rl_parameters):
    ...
    return n_active_params.reshape(n_participants)
...
for index_par, par in enumerate(participants):
    for it in range(iterations):
        ...
        active_params[index_par, par*it:par*(it+1), 0] = count_active_params(rl_parameters_dataset).numpy()
