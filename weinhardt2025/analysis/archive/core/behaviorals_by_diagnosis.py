...
DEFAULT_OUT = Path(
    "/Users/martynaplomecka/closedloop_rl/analysis/participants_analysis_dezfouli/plots/behaviorals_by_diagnosis"
)

DIAGNOSIS_GROUPS = ["Depression", "Bipolar", "Healthy"]
DIAGNOSIS_LABELS = ["Depression", "Bipolar", "Healthy"]
PALETTE = ["#FF6B6B", "#4ECDC4", "#45B7D1"]  # Red for Depression, Teal for Bipolar, Blue for Healthy
...
def load_data(csv_path: str | Path = "dezfouli_final_df_sindy_analysis_with_metrics.csv") -> pd.DataFrame:
    df = pd.read_csv(csv_path)
    if "diagnosis_group" not in df.columns:
...
def behavioural_boxplots(
    ...
    fig.suptitle(
        "Behavioural measures across diagnosis groups",
        fontsize=22, y=0.97,
    )
    axes = np.ravel([axes])
    ...
    for ax, beh in zip(axes, bmeas):
        data = [
            df.loc[df.diagnosis_group == g, beh].dropna()
            for g in range(len(DIAGNOSIS_GROUPS))
        ]
        bp = ax.boxplot(
            data,
            positions=range(len(DIAGNOSIS_GROUPS)),
            patch_artist=True,
            showmeans=False,
        )
        ...
        for i, grp in enumerate(data):
            ax.scatter(
                np.random.normal(i, 0.04, size=len(grp)),
        ...
        ax.set_xticks(range(len(DIAGNOSIS_GROUPS)))
        ax.set_xticklabels(DIAGNOSIS_LABELS, rotation=45)
    ...
    handles = [
        plt.Rectangle((0, 0), 1, 1,
                      ...
                      label=DIAGNOSIS_LABELS[i])
        for i in range(len(DIAGNOSIS_GROUPS))
    ]
    fig.legend(
        handles=handles,
    ...
    fpath = out_dir / f"behavioural_measures_by_diagnosis_{suffix}.png"
...
def summary_table(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    for beh in BEHAVIOURS:
        if beh not in df.columns:
            continue
        for g in range(len(DIAGNOSIS_GROUPS)):
            grp = df.loc[df.diagnosis_group == g, beh].dropna()
            if len(grp):
                rows.append(dict(
                    Measure=beh.replace("_", " ").title(),
                    Diagnosis_Group=DIAGNOSIS_LABELS[g],
                    Mean=grp.mean(),
    ...
    if rows:
        ...
        fpath = out_dir / "behavioural_measures_by_diagnosis_summary.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate behavioural box-plots by diagnosis group "
    ...
    parser.add_argument(
        "--csv",
        type=str, default="dezfouli_final_df_sindy_analysis_with_metrics.csv",
