...
all eckstein2026 params_array checkpoints (the sindy_threshold_pruning x
sindy_ensemble_pruning training HP scan).

For each checkpoint in params_array/ (stage1 checkpoints skipped), grid-searches
(alpha_W, alpha_H) at a fixed K_per_module and records train/test NLL/BIC via
...
Mirrors weinhardt2026/studies/dezfouli2019/analysis_compression_hpscan_array.py.

Usage:
    python weinhardt2026/studies/eckstein2026/analysis_compression_hpscan_array.py
"""
...
def load_estimator(path, n_actions):
...
def plot_across_models(df: pd.DataFrame, output_dir: str):
...
def main():
    ...
    # 12/16 models best at alpha_W=0.005 (upper edge) and 10/16 at alpha_H=0.0005 (upper edge).
    ...
    dataset_train, dataset_test = split_data_along_blockdim(dataset, (2,))
    ...
    for i, path in enumerate(pkl_paths):
        ...
        estimator = load_estimator(path, n_actions=n_actions)
    ...
    plot_across_models(combined, results_dir)
