"""eckstein2026 variant s5_choice_split_asym: Final model with value_choice split; chosen keeps action[t-1], not-chosen decays without input.
...
CONFIG = SpiceConfig(
    library_setup={
        'value_reward_chosen': ['reward[t]', 'value_reward_mean'],
        'value_reward_not_chosen': ['value_reward_mean'],
        'value_choice_chosen': ['action[t-1]'],
    ...
    states_in_logit=['value_reward', 'value_choice', 'value_exploration', 'bias_attention'],
)
...
class SpiceModel(BaseModel):
    """Final model with value_choice split; chosen keeps action[t-1], not-chosen decays without input."""
    ...
    def __init__(self, **kwargs):
        ...
        self.setup_module(key_module='value_choice_not_chosen', input_size=0, dropout=self.dropout)
