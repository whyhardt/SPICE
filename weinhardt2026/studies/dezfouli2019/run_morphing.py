...
Morphs participant embeddings along the avg_reward direction, refits SINDy
equations, generates behavior to validate the morphing direction, and reports
whether avg_reward increases monotonically along the morphing axis.
...
def main():
    ...
    # ── Run morphing ──
    save_path = os.path.join(STUDY_DIR, 'params', 'spice_dezfouli2019_morph_reward.pkl')
    ...
    # ── Extract morphed coefficients ──
    print("\nExtracting morphed coefficients...")
    ...
    # Save morphed coefficients for plotting
    ...
    if not args.skip_validation:
        ...
        # Need the morphed dataset for generation
        ...
        for run_i in range(args.n_validation_runs):
            ...
            avg_per_step, _ = _validate_morphing(
                result['estimator'], morphed_dataset,
        ...
        for i, (sv, ar, se) in enumerate(zip(result['step_values'], avg_reward_mean, avg_reward_se)):
            bar = '█' * int(ar * 200)
