...
class SpiceModel(BaseModel):
    ...
    def __init__(self, reward_binary: bool = False, **kwargs):
        ...
        self.setup_module(key_module='value_reward_not_chosen', input_size=0, embedding_size=self.embedding_size, dropout=dropout)  # -> 21 terms
    ...
    def forward(self, inputs, prev_state=None):
        ...
        for trial in spice_signals.trials:
            ...
            # BUFFER UPDATES:
            # REWARD BUFFER UPDATES: Shift reward buffer for chosen action, flush for not chosen action -> no stale reward buffers
            # ACTION BUFFER UPDATES: Shift all buffer entries according to action
            
            self.state['buffer_reward_3'] = self.state['buffer_reward_2'] * spice_signals.actions[trial]
            self.state['buffer_reward_2'] = self.state['buffer_reward_1'] * spice_signals.actions[trial]
            self.state['buffer_reward_1'] = spice_signals.rewards[trial]
